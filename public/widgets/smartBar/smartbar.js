  ...
  static serializeOptions(opt, templateId) {
    ...
    switch (templateId) {
      ...
      case "def-json_btn": {
        ...
        template = `${jstr}`;
        template += "\n\n";
        template += `// later, use static function SmartBars.JsonToOptions(options); to convert JSON string
...
`;
  ...
  _build() {
    ...
    this._bodyActiveBasis = SmartWidgets.addElement(
      "path",
      {
    ...
    if (this._o.isShowThr) {
      // In the case of allowed display of threshold values draw rect instead path
      this._bodyActive = SmartWidgets.addElement(
        "rect",
        {
    ...
    if (this._o.scalePosition !== "none") {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        this._scaleTextA[i] = SmartWidgets.addElement(
          "text",
          {
            class: "stbar",
  ...
  update(data = null) {
    // JSON object with defined progress:[]. In case of cfg={}, or opt:{} defined
    ...
    if (!data) {
      ...
      // show external or emulated data
      let options = null;
  ...
  generateExData() {
    ...
    const min = 0;
    const value = Math.abs(Math.floor(Math.random() * (max - min + 1) + min));
    ...
    const trend1 = Math.abs(Math.floor(Math.random() * (max - min + 1) + min));
    const trend2 = Math.abs(Math.floor(Math.random() * (max - min + 1) + min));
  ...
  getParams(filter = "all") {
    ...
    switch (filter) {
      ...
      case "json":
        ...
        return SmartBars.getJSON(opt);
      case "css":
        opt = SmartWidgets.getCustomParams(customProp, defOptions, this._o, "dirty", "none");
        return SmartBars.getCSS(opt);
      case "names":
