...
SmartGauges.findWidget(nameSubstr) - Finds and returns an object wich name contains specified substing from all founded widgets. 
A special case of "getWidget" function call, which widget will be searched by the it's name in all categories
...
where: contId    - container id assotiated with widget 
...
"targets" - used to transmit information about linking external data to a certain scale. 
            ...
            Notes: 
                1. This structure does not change the amount of the widget scales, but only establishes a connection
...
"specials" - Additional data structure, which describes special behavior of the widget. 
...
Note: All parameters - are optional and depends on the required changes!
