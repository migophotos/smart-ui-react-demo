...
class SmartWidgets {
  ...
  static get defaultDataFormat() {
    // deault data format section
    const df = {
      uuid: "$FULL_PATH_MD5",
      state: "$ALERT_STATE",
      color: "$ALERT_COLOR",
  ...
  static addElement(type, params, parent = null, doc = null) {
    if (!doc) {
      // try to main document in case of doc not specified
      if (parent) {
  ...
  _intervalTimer() {
    this._interval = setInterval(() => {
      for (let entry of this._heap.entries()) {
        ...
        if (obj && obj.getAlias() == this._alias && obj._o.isRun) {
          ...
          if (ic <= 0) {
            let data = null,
              updMode = obj.isEmulate(); // 1 - enabled, 0 - disabled, -1 - nulled
            if (updMode) {
              data = obj.generateExData();
              if (updMode === -1) {
                // clear all data and stop emulation
                obj.emulate(0);
              }
            }
            obj.update(data);
...
class ScrollableContainer {
  constructor(id = null, options = null) {
    ...
    this._clipFace = SmartWidgets.addElement(
      "clipPath",
      {
    ...
    this._face = SmartWidgets.addElement(
      "rect",
      {
    ...
    this._bodyActiveG = SmartWidgets.addElement(
      "g",
      {
        // the group that will be moved up / or down
    ...
    this._bodyActiveBkg = SmartWidgets.addElement(
      "rect",
      {
